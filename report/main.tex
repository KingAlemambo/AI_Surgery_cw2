...
For Task A, the model receives a sequence of frames along with \textbf{normalised elapsed time} (divided by maximum surgery duration) concatenated to CNN features before the LSTM. This temporal anchor helps the model understand where in the procedure it is, providing context that pure visual features lack.
...
\textbf{Time-Augmented Model:} We use our best-performing Task A model (LSTM with $T=30$, partial fine-tuning) to generate predicted surgery remaining time. This Task A model is \textbf{frozen during Task B training} (inference only), and its predictions are concatenated to CNN features before the LSTM. Critically, we use predicted time rather than ground-truth elapsed time, testing whether Task A estimates provide useful context in a realistic deployment scenario where true timing is unavailable.
...
The LSTM outperforms the Transformer across all metrics. Figure~\ref{fig:transformer} shows the Transformer's severe overfitting: training MAE drops to 1.79 min while validation remains at 7.85 min. For limited data and short sequences, LSTM's sequential inductive bias proves advantageous over learned attention patterns.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../src/plots_transformer/transformer_seq30_mae.png}
\caption{Transformer training curves showing severe overfitting: train MAE decreases while validation MAE remains flat, triggering early stopping after 4 epochs.}
...
Future work should explore: (1) improving duration prediction through multi-modal inputs (e.g., instrument kinematics), (2) uncertainty-aware time predictions that signal reliability to downstream models, and (3) end-to-end joint training rather than frozen task chaining.
