...
% ============================================================================
\begin{abstract}
Accurate prediction of surgical duration and tool usage is essential for operating room scheduling and intraoperative decision support. We present a CNN-LSTM framework for surgical workflow analysis using the Cholec80 dataset. For duration prediction (Task A), we achieve 5.18 minutes MAE for phase remaining and 10.83 minutes for surgery remaining on the test set. For tool detection (Task B), we investigate whether predicted timing from Task A improves performance. On the test set, augmenting with Task A predictions achieves the best mAP (0.962), outperforming both baseline (0.955) and ground-truth elapsed time (0.960)---demonstrating that task chaining between surgical AI models is effective.
...
Table~\ref{tab:task_a} shows LSTM results on validation and test sets. The LSTM outperforms the Transformer on validation (7.27 vs 7.59 min phase MAE); the Transformer exhibited severe overfitting, triggering early stopping after 4 epochs.

\begin{table}[h]
\centering
\caption{Task A duration prediction results (LSTM, $T=30$, MAE in minutes).}
\label{tab:task_a}
\begin{tabular}{lcccc}
\toprule
Set & Phase Rem. & Surgery Rem. & Phase Starts & Phase Ends \\
\midrule
Validation & 7.27 & 13.80 & 7.28 & 9.12 \\
Test & \textbf{5.18} & \textbf{10.83} & \textbf{5.05} & \textbf{6.90} \\
\bottomrule
\end{tabular}
\end{table}

Test set performance is notably better than validation, with surgery remaining MAE of 10.83 minutes ($\sim$20\% error for typical 50-minute procedures). Phase start/end predictions achieve 5--7 minute MAE, enabling useful surgical timeline estimation.

\subsection{Task B: Tool Detection}

\subsubsection{Time Augmentation Comparison}
Table~\ref{tab:taskb} compares tool detection across validation and test sets.

\begin{table}[h]
\centering
\caption{Task B tool detection results (mAP / Phase Accuracy).}
\label{tab:taskb}
\begin{tabular}{lcc}
\toprule
Model & Validation & Test \\
\midrule
Baseline (no time) & 0.960 / 89.2\% & 0.955 / 85.6\% \\
+ Elapsed time (GT) & 0.962 / 90.5\% & 0.960 / 84.4\% \\
+ Predicted time (Task A) & 0.954 / 88.6\% & \textbf{0.962} / \textbf{86.3\%} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding:} While validation suggested predicted time hurts performance (0.954 vs 0.960), the \textbf{test set reveals the opposite}: predicted time achieves the highest mAP (0.962), outperforming both baseline (0.955) and ground-truth elapsed time (0.960). This supports our hypothesis that Task A predictions provide useful temporal context for downstream tasks.

Figure~\ref{fig:test_results} summarises test set performance for both tasks.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/test_results_summary.png}
\caption{Test set results. Left: Task A duration prediction MAE. Right: Task B tool detection mAP---predicted time from Task A achieves the best performance.}
\label{fig:test_results}
\end{figure}

\subsubsection{Per-Tool Analysis}
On the test set, Hook achieves highest AP (0.996) due to its distinctive appearance. Grasper has lower AP (0.889) as it appears throughout surgery, making absence hard to predict. All tools exceed 0.88 AP, demonstrating robust multi-label detection.
...
\textbf{RQ2 (Phase Timeline Prediction):} Multi-output prediction successfully estimates complete surgical timelines. On the test set, phase start/end predictions achieve 5--7 minute MAE, with surgery remaining at $\sim$11 minutes ($\sim$20\% error). This granularity is valuable for OR scheduling---knowing when specific phases will occur enables targeted resource preparation.

\textbf{RQ3 (Task Chaining):} Our central hypothesis---that predicted temporal information improves downstream tasks---\textbf{is supported on the test set}. While validation results were inconclusive (predicted time appeared to hurt), test set evaluation shows predicted time achieves the best mAP (0.962), outperforming both baseline (0.955) and ground-truth elapsed time (0.960). This demonstrates that Task A predictions, despite $\sim$20\% error, provide useful temporal context that improves tool detection in practice.
...
\textbf{Limitations:} Single-dataset evaluation (Cholec80) limits generalisability. Class imbalance affects rare tools despite weighting. 1 fps sampling may miss brief tool appearances. The discrepancy between validation and test results warrants further investigation with additional datasets.
...
We presented a CNN-LSTM framework for surgical workflow analysis on Cholec80. Task A (duration prediction) achieves $\sim$11 minute MAE for surgery remaining time and 5--7 minute MAE for phase start/end predictions on the test set, enabling useful surgical timeline estimation. Task B (tool detection) achieves strong performance (0.962 mAP), with \textbf{predicted temporal context from Task A improving over both baseline and ground-truth elapsed time}.

Our key finding is that \textbf{task chaining between surgical AI models is effective}: duration predictions, despite $\sim$20\% error, provide sufficient temporal context to improve downstream tool detection. This validates the approach of building modular surgical AI pipelines where upstream predictions enhance downstream tasks.

Future work should explore: (1) improving duration prediction through multi-modal inputs (e.g., instrument kinematics), (2) uncertainty-aware predictions that signal reliability to downstream models, and (3) \textbf{surgical anomaly detection}---leveraging the learned tool-phase correlations to flag when a tool appears in an unexpected phase, potentially indicating procedural errors or complications.
