...
\textbf{Clinical context:} Surgery remaining MAE of $\sim$14 minutes represents approximately 25\% error for typical cholecystectomies (45--60 minutes)---useful for coarse scheduling but highlighting the inherent difficulty of duration prediction from visual information alone.
...
\textbf{Limitations:} Single-dataset evaluation (Cholec80) limits generalisability. Class imbalance affects rare tools despite weighting. 1 fps sampling may miss brief tool appearances. The $\sim$25\% duration prediction error may be an inherent limitation of visual-only approaches.
